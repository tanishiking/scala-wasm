...
// Copied from https://github.com/scala-js/scala-js/blob/95c38be0501fb81c992f3ce8b9e5a9828c2e831e/test-suite/shared/src/test/scala/org/scalajs/testsuite/compiler/ReflectiveCallTest.scala
// We can remove this test once all tests in ReflectiveCallTest.scala passes
/*
...
object ReflectiveCallTest {
  def main(): Unit = {
    ...
    thisTypeInReturnTypes()
    genericReturnTypes()
    unaryMethodsOnPrimitiveTypes()
    binaryOperatorsOnPrimitiveTypes()
    qualityOperatorsOnPrimitiveTypes()
    compareToForPrimitives()
    concatForPrimitives()
    arrays() // TODO: clone
    arraysOfPrimitiveValues() // TODO: clone
    strings()
    forwardersForInheritedMethods()
    bugCompatibleWithScalaJVMForInheritedOverloads()
    // javaLangObjectNotifyNotifyAll_Issue303()
    // javaLangObjectClone_Issue303()
    // scalaAnyRefEqNeSynchronized_Issue2709() assertThrows
    anyValEqNeSynchronized_Issue2709()
    // javaLangFloatDoubleIsNaNIsInfinite()
    defaultArguments_Issue390()
    unboxAllTypesOfArguments_Issue899()
  }

  def assertEquals(expected: Any, actual: Any): Unit = ok(expected == actual)
  def assertTrue(value: Boolean) = ok(value)
  def assertFalse(value: Boolean) = ok(!value)

  def subtypingInReturnTypes(): Unit = {
    class A { def x: Int = 1 }
  ...
  def thisTypeInReturnTypes(): Unit = {
  ...
  def unaryMethodsOnPrimitiveTypes(): Unit = {
  ...
  def binaryOperatorsOnPrimitiveTypes(): Unit = {
  ...
  def qualityOperatorsOnPrimitiveTypes(): Unit = {
  ...
  def compareToForPrimitives(): Unit = {
  ...
  def arraysOfPrimitiveValues(): Unit = {
  ...
  def forwardersForInheritedMethods(): Unit = {
    trait A {
      def foo: Int
    }
  ...
  def bugCompatibleWithScalaJVMForInheritedOverloads(): Unit = {
    class Base {
  ...
  /** def javaLangObjectNotifyNotifyAll_Issue303(): Unit = { type ObjNotifyLike = Any { def
    * notify(): Unit def notifyAll(): Unit } def objNotifyTest(obj: ObjNotifyLike): Int = {
    * obj.notify() obj.notifyAll() 1 }
    *
    * class A
    *
    * val a = new A() a.synchronized { assertEquals(1, objNotifyTest(a)) } }
    */

  /*
  def javaLangObjectClone_Issue303(): Unit = {
  ...
  /*
  def scalaAnyRefEqNeSynchronized_Issue2709(): Unit = {
  ...
  def anyValEqNeSynchronized_Issue2709(): Unit = {
    type ObjWithAnyRefPrimitives = Any {
      def eq(that: AnyRef): Boolean
      def ne(that: AnyRef): Boolean
      def synchronized[T](f: T): Any
  ...
  def defaultArguments_Issue390(): Unit = {
  ...
  def unboxAllTypesOfArguments_Issue899(): Unit = {
    class Foo {
      def makeInt: Int = 5
